...
function Chat({ boardId, isOpen, onClose }) {
  // ####################### STATE MANAGEMENT #########################
  const [messages, setMessages] = useState([]);         // lista svih poruka
  const [newMessage, setNewMessage] = useState('');     // trenutni tekst koji korisnik tipka
  const [loading, setLoading] = useState(true);         // indikator učitavanja dok se poruke dohvaćaju s API-ja
  const [user, setUser] = useState(null);               // podatci o korisniku
  const messagesEndRef = useRef(null);                  // referenca na prazan div na dnu za auto-scroll
  ...
  useEffect(() => {
    ...
    const fetchMessages = async () => {
      try {
        ...
        scrollToBottom(); // scroll odmah nakon fetch-a
      } catch (error) {
    ...
    const setupSocket = () => {
      const echo = getEcho();
      if (!echo) {
        console.warn('❌ Echo not ready, retry in 500ms');
        timeoutId = setTimeout(setupSocket, 500); // retry
        return;
      }

      // echo je spreman → kreiram channel
  ...
  const handleSendMessage = async () => {
    ...
    try {
      const response = await api.post(`/boards/${boardId}/messages`, {
